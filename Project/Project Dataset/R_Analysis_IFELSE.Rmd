...
```{r}
  ...
  df.original$is_long_review <- ifelse(nchar(df.original$review)>120,1,0)
```
...
```{r}
  #Long reviews found helpful or unhelpful
  barplot(with(df.original, table(num_found_helpful, is_long_review)), main = "Long Reviews found Helpful or Unhelpful", col=1)

  #Short reviews found funny
  barplot(with(df.original, table(num_found_funny, is_long_review)), main = "Long Reviews found Funny or Unfunny", col=1)
```
